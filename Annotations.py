...
def boundingBox(path):
    for filename in glob.glob(path):
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))  #bbox shape
        grad = cv2.morphologyEx(small, cv2.MORPH_GRADIENT, kernel)  #outline of the image

        _, bw = cv2.threshold(grad, 0.0, 255.0, cv2.THRESH_BINARY | cv2.THRESH_OTSU)  

        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (9, 1)) 
        connected = cv2.morphologyEx(bw, cv2.MORPH_CLOSE, kernel)  #Closing small gaps or holes  

        contours, hierarchy = cv2.findContours(connected.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE) #finding white object from black background
